...
describe('wasm metabolism core (zero-copy shared memory)', () => {
  ...
  it('stays identical with a cognition cost on (metabolism falls back to TS, shared RNG)', () => {
    const p = params({ seed: 17, initialPopulation: 80, foodAbundance: 350, cognitionCost: 1.5 });
    const ts = createSimulation(p);
    const wasm = createSimulation(
      p,
      undefined,
